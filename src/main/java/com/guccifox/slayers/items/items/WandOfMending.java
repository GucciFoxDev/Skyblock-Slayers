...
public class WandOfMending extends CustomItem {
    public WandOfMending(int id, Rarity rarity, String name, Material material, int durability, boolean stackable, boolean oneTimeUse, boolean hasActive, List<ItemAbility> abilities, int manaCost, boolean reforgeable, ItemType itemType, boolean glowing) {
    ...
    public void leftClickAirAction(Player player, ItemStack paramItemStack) {
        if(!(ItemUtilities.enforceCooldown(player, "medium_heal", 1d, paramItemStack, false))) {
            ItemUtilities.warnPlayer(player, Collections.singletonList(ChatColor.RED + "You are on cooldown!"));
            return;
        }
        Utilities.sendActionbar(player, ChatColor.AQUA + "-80 Mana (" + ChatColor.GOLD + "Medium Heal" + ChatColor.AQUA + ")");
        int i = Bukkit.getScheduler().scheduleSyncRepeatingTask(Slayers.getPlugin(), new Runnable() {
            ...
            public void run() {
                if (player.getHealth() < 18.5) {
                    ...
                    Utilities.sendActionbar(player, "§c+1.5❤");
                } else {
                    player.sendMessage(ChatColor.RED + "You can not heal any more!");
                }
            }
        }, 0, 20L);
    ...
    @Override
    public void leftClickBlockAction(Player player, PlayerInteractEvent paramPlayerInteractEvent, Block paramBlock, ItemStack paramItemStack) {
        if(!(ItemUtilities.enforceCooldown(player, "medium_heal", 1d, paramItemStack, false))) {
            ItemUtilities.warnPlayer(player, Collections.singletonList(ChatColor.RED + "You are on cooldown!"));
            return;
        }
